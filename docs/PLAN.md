...
**Design backbone — ASPICE.** SWE.1 requirements: `docs/requirements*.md`
(see `requirements_index.md`). SWE.2 architecture: `docs/architecture.md`.
SWE.3 detailed design: added per stage as it is built. SWE.4–6: host tests,
integration, qualification, gated in CI.

**Execution — layer by layer, commit by commit.** Each stage is one vertical
...
Requirements: `docs/requirements_gateway.md`. Each milestone gate = rebuild the
load-bearing pieces solo + defend cold.

## Planned

- **Linux character-device driver (M4)**: `/dev/sharak0` exposing
  open/read/write/release, built against kernel headers (`docs/requirements_driver.md`).
...
- **Sensor-agnostic core.** Transport and FrameParser never open the payload, so
  ...
  learn a new type (`requirements_gateway.md` §0).
